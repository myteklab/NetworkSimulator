...
var Connectable = function(c_owner, macmode, ipmode, limitbroadcast, performNAT)
{
    ...
    this.getDstMAC = function(ip)
    {
        ...
        {
            var gatewayIP = gateways.getGatewayForIP(ip, null);
            result = this.findMACforIP(gatewayIP);
        }
        return result;
    };
    ...
    this.findMACforIP = function(ip, exclude)
    {
        ...
        if (ip !== null)
        {
          // First pass: Check if this IP is directly connected to one of our interfaces
          // If so, ONLY query that specific interface to avoid routing loops
          var directInterface = -1;
          for (var j = 0; j < connectors.length; j++)
          {
              if (connectors[j] !== exclude)
              {
                  var ipinfo = this.getIPInfo(j);
                  if (ipinfo && ipinfo.getIPv4() !== null && ipinfo.sameNetwork(ip))
                  {
                      directInterface = j;
                      break;
                  }
          ...
          while ((result === null) && (i < connectors.length))
          {
              if (connectors[i] !== exclude)
              {
                  ...
                  if (c !== null)
                  {
                      // Prevent infinite loop: Don't ask through our own other interfaces
                      // This happens when multiple interfaces of a router/device are on the same switch
                      var connectedOwner = c.getOwner ? c.getOwner() : null;
                      if (connectedOwner === this.getOwner())
                      {
                          // Skip - this would ask through our own device's other interface
                          i++;
                          continue;
                      }
                      ...
                      // For devices with no IP (like switch ports or router WAN ports without IP),
